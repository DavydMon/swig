...
-(void)configure:(SWAccountConfiguration *)configuration completionHandler:(void(^)(NSError *error))handler {
    ...
    NSString *domain = self.accountConfiguration.domain;

    
    NSRegularExpression *findRegex = [NSRegularExpression regularExpressionWithPattern:@"[^:]*" options:0 error:nil];
    NSTextCheckingResult *match = [findRegex firstMatchInString:domain options:0 range:NSMakeRange(0, [domain length])];

    int numberOfMatches = [match numberOfRanges];

    NSString *hostname = domain;
    if (numberOfMatches > 0) {
        hostname = [domain substringWithRange:[match rangeAtIndex:0]];
    }

    pj_str_t host = [hostname pjString];
    ...
    acc_cfg.reg_uri = [[SWUriFormatter sipUri:self.accountConfiguration.address withDisplayName:self.accountConfiguration.displayName] pjString];
    ...
    BOOL isMediaIpv6Enabled = NO;

    pj_sockaddr sockAddr;
    pj_status_t rc;
    rc = pj_getdefaultipinterface(pj_AF_INET6(), &sockAddr);
    if (rc != PJ_SUCCESS) {
        char errbuf[80];
        pj_strerror( rc, errbuf, sizeof(errbuf));
    }
    else {
        isMediaIpv6Enabled = YES;
    }
    ...
    acc_cfg.ipv6_media_use = isMediaIpv6Enabled ? PJSUA_IPV6_ENABLED : PJSUA_IPV6_DISABLED;
    NSLog(@"<--ipv6_media_use--> %d", acc_cfg.ipv6_media_use);
...
-(void)endAllCalls {
    ...
    for (SWCall *call in self.calls) {
        if (call.callState != SWCallStateDisconnectRingtone) {
            [call hangup:nil];
        }
    }
}
...
-(void)makeCall:(NSString *)URI toGSM:(BOOL) isGSM withVideo:(BOOL) withVideo withCallId: (NSString *)generatedCallId completionHandler:(void(^)(NSError *error))handler {
    ...
    [thrManager runBlock:^{
        ...
        NSString *uri = [NSString stringWithFormat:@"<sips:%@@%@%@>",URI,self.accountConfiguration.domain,gsmPostfix];
...
static void sendMessageCallback(void *token, pjsip_event *e) {
    ...
    if (e->body.tsx_state.type != PJSIP_EVENT_RX_MSG) {
        ...
        int accountID = [SWEndpoint sharedEndpoint].firstAccountId;
