...
static void refer_notify_callback(void *token, pjsip_event *e) {
    ...
    if (via_hdr) {
        ...
        NSError *error = nil;
        int statusCode = e->body.tsx_state.tsx->status_code;
        
        if (statusCode > 399) {
            error = [NSError errorWithDomain:@"SWIG Notify error" code:statusCode userInfo:nil];
        }
        
        pjsip_cid_hdr *callIdHeader = (pjsip_cid_hdr*)e->body.rx_msg.rdata->msg_info.cid;
        if (callIdHeader) {
            NSString *callId = [NSString stringWithPJString:callIdHeader->id];
            
            SWErrorObjBlock handler = [[SWEndpoint sharedEndpoint].referAnswerHandlers objectForKey:callId];
            if (handler != nil) {
                [[SWEndpoint sharedEndpoint].referAnswerHandlers removeObjectForKey:callId];
                handler(error);
            }
        }
    }
    
    NSLog(@"<--notifyAnswerTest-->refer_notify_callback completed");
}
...
-(instancetype)init {
    ...
    self.referAnswerHandlers = [NSMutableDictionary new];
...
- (void) incomingRefer:(pjsip_rx_data *)data {
    ...
    [self innerOnReferFrom:[NSString stringWithPJString:from->user] server:serverAddress forAccount:acc_id ipV6:NO callId:[NSString stringWithPJString:data->msg_info.cid->id] cSeq:0 handler: nil];
}
...
- (void) onReferFrom: (NSString *) abonent server: (NSString *)voiceUrl forAccount: (int)acc_id ipV6: (BOOL) ipv6Enabled callId: (NSString *)callId cSeq: (int)cSeq handler: (SWErrorObjBlock)handler {
    ...
    if([NSThread currentThread] != regThread) {
        [thrManager runBlock:^{
            [self innerOnReferFrom:abonent server:voiceUrl forAccount:acc_id ipV6:ipv6Enabled callId:callId cSeq:cSeq handler: handler];
        } onThread:regThread wait:NO];
        return;
    }
    else {
...
- (void) innerOnReferFrom: (NSString *) abonent server: (NSString *)voiceUrl forAccount: (int)acc_id ipV6: (BOOL) ipv6Enabled callId: (NSString *)callId cSeq: (int)cSeq handler: (SWErrorObjBlock)handler {
    ...
    cid_hdr->id = [callId pjString];
    pjsip_msg_add_hdr(pjsua_data->msg, cid_hdr);
    
    if (handler) {
        [self.referAnswerHandlers setObject:handler forKey:callId];
    }
    ...
    if (status != PJ_SUCCESS) {
        if (handler) {
            [self.referAnswerHandlers removeObjectForKey:callId];
            handler([NSError errorWithDomain:@"SWIG error sending notify" code:status userInfo:nil]);
        }
        return;
    }
}
